...
- Advanced page key UX after the post-v1 fix: invalid/unreachable errors, redacted field, no-key warning, Replace/Remove (visual pass + Vietnamese copy).
...
- `commands.rs`: one setter command per setting; each returns the updated `AppSettings`. `set_groq_api_key` trims, validates via Groq `GET /models` (`cleanup::validate_api_key`), and saves only on 200; errors `empty_key` / `invalid_key` (401/403) / `unreachable`. The Advanced page never receives the key: a saved key shows as a fixed disabled `gsk_••••` placeholder with Replace/Remove. `set_clean_and_reformat(true)` fails with `no_api_key` until a key exists; `clear_groq_api_key` also turns Clean and Reformat off.
