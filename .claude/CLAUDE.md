...
- Open source (2026-09-18): MIT `LICENSE` (Ken Nguyen), `license = "MIT"` in `Cargo.toml`/`package.json`, and a public `README.md` (features, install, privacy, build from source, credits). The repo is published as `audibl` on GitHub, with the DMG attached to a `v1.0.0` GitHub Release; `INSTALL.md` is the tester install guide the README links to.
