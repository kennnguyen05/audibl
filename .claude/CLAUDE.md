...
- Open source (2026-09-18): MIT `LICENSE` (Ken Nguyen), `license = "MIT"` in `Cargo.toml`/`package.json`, and a public `README.md` (features, install, privacy, build from source, credits). Published at https://github.com/kennnguyen05/audibl (public), with the DMG and `INSTALL.md` attached to the `v1.0.0` release; for a new release, bump the version, `bun run release`, then `gh release create vX.Y.Z <dmg> INSTALL.md`. `gh` has no config on this Mac, so run it with `GH_TOKEN` taken from `git credential fill` (the osxkeychain GitHub credential); `INSTALL.md` is the tester install guide the README links to.
