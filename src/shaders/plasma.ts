/**
 * The "Plasma" background shader, made with the 21st.dev Shader Builder.
 *
 * The fragment source is kept verbatim — it is generated upstream code, not
 * ours, and every knob it exposes is driven from `ShaderBackground.tsx`
 * through the packed uniforms documented in its own header comment. Do not
 * edit the GLSL to retune the look; change the uniform values instead.
 */

/** A single triangle large enough to cover the clip volume — no vertex buffer
 *  maths, no quad, and one less fragment-shader invocation per pixel than two
 *  triangles meeting on the diagonal. */
...
//   u_scene = vec4(canvas width, canvas height, seconds * 0.57, 4.0)
//   u_shape = vec4(1.26, 0.00, 0.28, 0.00)
//   u_surface = vec4(1.82, 1.50, -0.02, 1.04)
//   u_finish = vec4(5.36, 0.29, 0.006, 0.04)
//   u_transform = vec4(1503.0, 0.00, 0.06, 0.0)
//   u_space = vec4(-0.20, 0.04, pointer x, pointer y)
//   u_cursor = vec4(presence, 0.0, 0.48, 0.38)
