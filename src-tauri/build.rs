...
/// Generates `TrayStrings` and the per-language `TRANSLATIONS` map from the
/// frontend locale files, so the menu bar menu uses the same copy as the UI.
///
/// Source of truth: `src/i18n/locales/*/translation.json`. The English `tray`
/// section defines the struct fields.
